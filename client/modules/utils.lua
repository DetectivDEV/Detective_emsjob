...
function utils.addRemoveItem(type, item, quantity)
    ...
    TriggerServerEvent("detective_emsjob:removAddItem", data)
end
...
function utils.useItem(item, value)
    ...
    data.item = item
    data.value = value
    TriggerServerEvent("detective_emsjob:useItem", data)
end

function utils.getItem(name)
    local item = lib.callback.await('detective_emsjob:getItem', false, name)
...
RegisterNetEvent('detective_emsjob:showNotification', utils.showNotification)
