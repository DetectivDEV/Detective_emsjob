...
local function checkPatient(target)
    ...
    local data = lib.callback.await('detective_emsjob:getData', false, targetServerId)
    ...
    if isDead then
        options[#options + 1] = {
            ...
            onSelect = function()
                ...
                TriggerServerEvent("detective_emsjob:healPlayer", dataToSend)
            end,
        }
...
function createDistressCall()
    ...
    local input = lib.inputDialog('Detective EMS', {
    ...
    if not Config.UseInterDistressSystem then
        ...
        TriggerServerEvent("detective_emsjob:createDistressCall", data)
    end
...
function openDistressCalls()
    ...
    local distressCalls = lib.callback.await('detective_emsjob:getDistressCalls', false)
                ...
                lib.registerContext({
                    ...
                    options = {
                        ...
                        },
                        {
                            ...
                            onSelect    = function()
                                TriggerServerEvent("detective_emsjob:callCompleted", call)
...
RegisterNetEvent('detective_emsjob:useEmsTablet', openDistressCalls)
...
RegisterNetEvent("detective_emsjob:playHealAnim", function(data)
    local playerPed = cache.ped or PlayerPedId()
...
RegisterNetEvent("detective_emsjob:createDistressCall", function(name)
    if not hasJob(Config.EmsJobs) then return end
