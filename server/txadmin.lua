...
AddEventHandler('txAdmin:events:healedPlayer', function(eventData)
    ...
    if target ~= -1 then
        ...
        TriggerClientEvent('detective_emsjob:healPlayer', target, { revive = playerStateDead })
    elseif target == -1 then
        for _, playerId in ipairs(GetPlayers()) do
            local playerStateDead = Player(playerId).state.dead
            TriggerClientEvent('detective_emsjob:healPlayer', playerId, { revive = playerStateDead })
        end
    end
end)
